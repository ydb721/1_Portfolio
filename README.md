...
> 이 브랜치는 과제 1의 `index.html`을 보존하고 하단에 비공개 영역을 추가한 **검토용 구현**이다. 기존 공개 결과물은 <https://skt-aleph-01.vercel.app/>이다. **이 브랜치가 해당 주소에 배포됐다는 확인은 없다.** 영속 저장소 설정과 실제 기기에서의 두 패스키 등록 전에는 완주 제출물로 쓰면 안 된다.
...
브라우저 WebAuthn API와 Node.js `node:crypto`를 사용해 **직접 구현**했다. 등록 질문·로그인 질문·세션·공개키·가상 자료는 영속 Redis REST 저장소에 저장한다. `tests/evidence.json`의 `storedPublicKeyJWK`는 테스트 과정에서 서버 저장소 모형에 실제 저장한 `kty`, `crv`, `x`, `y` 공개키 좌표다. 비밀번호가 아니다. 등록 요청 본문은 `ceremony`, `credential.id`, `response.clientDataJSON`, `response.attestationObject`로 구성되고 개인키 필드가 없다. 실제 기기에서는 개인키로 서명하지만 개인키 자체를 API에 전송하지 않는다. 자동 검사에서만 테스트 프로세스가 임시 개인키를 만든다. **실제 패스키 저장 위치(구글 비밀번호 관리자·기기 자체·보안 키)는 미확인이다.**
...
AI가 공개 페이지에 잠긴 영역과 서버 흐름, 합성 키 테스트, 설명서 초안을 작성했다. 사용자 판단으로 확인해야 할 실제 저장 기기, 최초 계정명, 운영 저장소, Vercel 배포 상태를 AI가 대신 결정한 것처럼 기록하지 않았다.
...
1. 어디로: 원본 공개 사이트 <https://skt-aleph-01.vercel.app/>. **이 변경의 공개 배포 URL은 아직 검증하지 못했다.**
2. 세 단계: 배포·저장소 설정 후 새 계정에 첫 패스키 등록 → 패스키 로그인 → 가상 메모 세 건 보기.
...
2. 내가 직접 판단한 일: 실제 기기·저장 위치·운영 DB·최초 등록 방식은 아직 사용자가 판단해야 하며 선택했다고 적지 않음.
3. AI 제안을 따르지 않은 일: 실제 등록이나 Vercel 배포를 하지 못했는데 완료 증거로 쓰지 않음.

## 완주 체크리스트

| 항목 | 상태 |
| --- | --- |
| 과제 1 공개 카드와 동작 유지, 별도 비공개 경계 | 코드에 반영; 실제 배포 화면 미확인 |
| 비공개 가상 항목 3건, 미인증 401/HTML 미노출 | 합성 테스트 통과 |
| 등록·로그인 질문 매번 새 값, 1회 소진, 서명 성공/실패 | 합성 테스트 통과 |
| 공개키 저장, 이름·등록일 목록, 취소 시 미저장 | 합성 테스트 통과; 실기기 미등록 |
| 계정 2개·한 계정 키 2개·삭제 후 로그인·양방향 격리 | 합성 테스트 통과; 실기기 2개와 실제 Redis 미검증 |
...
본인 기기에서 실제 패스키 두 개를 등록하고 하나를 삭제한 뒤 남은 키로 로그인한 캡처·응답과 저장 위치를 기입해야 과제를 완성할 수 있다.
