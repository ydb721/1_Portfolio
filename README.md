...
브라우저 WebAuthn API와 Node.js `node:crypto`를 사용해 **직접 구현**했다. 등록 질문·로그인 질문·세션·공개키·가상 자료는 영속 Redis REST 저장소에 저장한다. `tests/evidence.json`의 `storedPublicKeyJWK`는 테스트 과정에서 서버 저장소 모형에 실제 저장한 `kty`, `crv`, `x`, `y` 공개키 좌표다. 비밀번호가 아니다. 등록 요청 본문은 `ceremony`, `credential.id`, `response.clientDataJSON`, `response.attestationObject`로 구성되고 개인키 필드가 없다. 실제 기기에서는 개인키로 서명하지만 개인키 자체를 API에 전송하지 않는다. 자동 검사에서만 테스트 프로세스가 임시 개인키를 만든다. **실기기 저장 위치**: 첫 키 `노트북`은 Google 비밀번호 관리자, 둘째 키 `Windows Hello`는 Windows 장치에 저장했다. 이 둘은 하나의 `sample_a` 가상 계정에 등록했고, 첫 키를 서버에서 삭제한 뒤 둘째 키로 로그인했다. 공개키 원문은 테스트 저장소 모형에서만 확인했으며 운영 Redis의 공개키 덤프는 아직 남기지 못했다.
...
**실제 Preview 화면 확인(2026-09-25, 화면 관찰)**: 로그인 전 `GET /api/gateway?view=me`에서 `{ "handle": null }` 표시; Google 비밀번호 관리자에 `sample_a`의 `노트북` 등록 후 패스키 로그인하여 가상 메모 3건과 키 이름·날짜 표시. 두 번째 등록 창 취소 뒤 목록에 새 키가 없고 취소 안내 표시. Windows Hello에 같은 계정의 두 번째 키를 등록하여 목록에 2건과 날짜 표시. 서버 목록에서 첫 키 `노트북` 삭제 후 1건만 표시; 로그아웃 시 가상 메모가 화면에서 사라지고 남은 Windows Hello로 로그인 후 다시 3건 표시. 이는 실제 기기/Redis에서 본 화면이며 아래의 HTTP 상태 코드 기록과 구분한다. 개인 이메일이 보이는 인증기 선택 창은 제출 증거에 넣지 않는다. **삭제한 키로 서버 로그인 거절, 다른 계정의 양방향 격리, 질문 재사용의 실서비스 HTTP 요청·응답은 아직 따로 수집하지 않았다.**
...
2. 내가 직접 판단한 일: 가상 계정명 `sample_a`, Upstash Free, Google 비밀번호 관리자와 Windows Hello에 각 키 저장, 첫 키 삭제 뒤 재로그인.
...
| 계정 2개·한 계정 키 2개·삭제 후 로그인·양방향 격리 | 실제 `sample_a`의 2키 등록·1키 삭제·남은 키 로그인 확인; 다른 계정은 합성 테스트만 |
...
남은 작업: 사용자 기기에 하나만 남은 `sample_a`의 백업 키 다시 등록, 실제 두 번째 가상 계정에서의 요청·응답, 질문 재사용/삭제 키 거절의 운영 응답, 시크릿 창 공개 접근 및 운영 사이트 반영 여부 결정. 화면 캡처에는 이메일·세션·토큰·PIN을 포함하지 않는다.
