...
async function api(path, options = {}) {
  ...
  if (!res.ok) throw Error(data.error || '요청에 실패했어.');
  return data;
}
...
async function register(handle, name) {
  ...
  catch (error) { status('등록을 취소했어. 서버에 새 패스키는 저장되지 않았어.'); return; }
  ...
  status('등록했어. 이제 패스키로 로그인해 줘. 패스키 목록은 로그인한 뒤 볼 수 있어.');
...
async function login(handle) {
  ...
  catch { status('패스키 로그인을 취소했어. 다시 시도할 수 있어.'); return; }
...
async function refresh() {
  ...
  $('#passkey-list').replaceChildren(...me.passkeys.map(key => {
    ...
    button.addEventListener('click', async () => {
      if (!confirm(`${key.name} 패스키를 계정에서 삭제할까?`)) return;
      try { await api(`/api/gateway?view=passkey&id=${encodeURIComponent(key.id)}`, { method: 'DELETE' }); status('패스키를 삭제했어.'); await refresh(); }
...
async function action(event, callback) {
  ...
  try {
    if (!window.PublicKeyCredential || !window.isSecureContext) throw Error('HTTPS와 패스키 지원 브라우저가 필요해.');
...
$('#passkey-logout').addEventListener('click', async () => {
  try { await post('/api/gateway?view=logout', {}); await refresh(); status('로그아웃했어. 비공개 내용은 화면에서 지웠어.'); }
  catch (error) { status(error.message); }
});
